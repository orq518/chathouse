...
public class Jewels extends BaseGameActivity implements IOnSceneTouchListener,
	...
	private TextureRegion mBonusStaticBGTextureRegion, mBonusBGTextureRegion,
			mBonusTextureRegion;
	/** LongestChain字体 **/
	...
	public Engine onLoadEngine() {
		...
		return new Engine(new EngineOptions(true, ScreenOrientation.PORTRAIT,
				...
				this.mCamera).setNeedsSound(true));// 需要播放音效
	}
	...
	public void onLoadResources() {
		...
		this.mBackground2Texture = new Texture(512, 1024,
		...
		this.mBackground2TextureRegion = TextureRegionFactory.createFromAsset(
				this.mBackground2Texture, this,
				"title_bg" + String.valueOf(this.mCurBGNum) + ".png", 0, 0);
		...
		this.mBonusStaticBGTextureRegion = TextureRegionFactory
				.createFromAsset(this.mBonusStaticBGTexture, this, "bonus.png",
						0, 0);
		this.mEngine.getTextureManager()
				.loadTexture(this.mBonusStaticBGTexture);
		...
		this.mScoreFont = FontFactory.createFromAsset(this.mScoreFontTexture,
				this, "fonts/bluehigh.ttf", 44, true, Color.WHITE);
		...
		this.mChapterFont = FontFactory.createFromAsset(this.mChapterTexture,
				this, "fonts/bluehigh.ttf", 44, true, Color.GREEN);
	...
	public boolean onSceneTouchEvent(final Scene pScene,
		...
		if (pSceneTouchEvent.getX() > 0
				&& pSceneTouchEvent.getX() < CAMERA_WIDTH
				&& pSceneTouchEvent.getY() > 0
				&& pSceneTouchEvent.getY() < CAMERA_WIDTH) {
	...
	private void initBonus() {
		// 静态文字背景
		final Sprite bonusBG = new Sprite(10, 425,
				this.mBonusStaticBGTextureRegion);
		...
		this.mChapterText.setBlendFunction(GL10.GL_SRC_ALPHA,
		...
		this.mXText.setBlendFunction(GL10.GL_SRC_ALPHA,
				GL10.GL_ONE_MINUS_SRC_ALPHA);
	...
	private void initLongestChain() {
		// Longest Chain
		final Text longestChain = new Text(146, 422, this.mLongestChainFont,
				"Longest Chain:", HorizontalAlign.LEFT);
		longestChain.setBlendFunction(GL10.GL_SRC_ALPHA,
				GL10.GL_ONE_MINUS_SRC_ALPHA);
		longestChain.setAlpha(1);
		longestChain.setScale(0.55f, 1.2f);
		this.mMainScene.getLayer(LAYER_SCORE).addEntity(longestChain);
		// 动态数字---最长连接
		this.mLongestChainText = new ChangeableText(293, 419,
				this.mChapterFont, "0");
		this.mLongestChainText.setBlendFunction(GL10.GL_SRC_ALPHA,
				GL10.GL_ONE_MINUS_SRC_ALPHA);
	...
	private void initScore() {
		this.mScoreBGText = new ChangeableText(170, 336, this.mScoreFont,
				"000000");
		this.mScoreBGText.setBlendFunction(GL10.GL_SRC_ALPHA,
				GL10.GL_ONE_MINUS_SRC_ALPHA);
	...
	private void initCellBG() {
		...
		for (int i = 0; i < CELLBG_HORIZONTAL; i++) {
			for (int j = 0; j < CELLBG_VERTICAL; j++) {
				cellBG[i][j] = new BackgroundCell(i, j,
						this.mBGCellTextureRegion);
				cellBG[i][j].setBlendFunction(GL10.GL_SRC_ALPHA,
						GL10.GL_ONE_MINUS_SRC_ALPHA);
	...
	private void initJewels() {
		...
		for (int i = 0; i < CELLS_HORIZONTAL; i++) {
			for (int j = 0; j < CELLS_VERTICAL; j++) {
				...
				while (checkHorizontal(value).size() >= 3
						|| checkVertical(value).size() >= 3) {
	...
	private void onStarting() {
		// 阴影效果
		final Sprite background2 = new Sprite(0, 0,
				this.mBackground2TextureRegion);
		background2.setAlpha(0.7f);
		background2.setBlendFunction(GL10.GL_SRC_ALPHA,
				GL10.GL_ONE_MINUS_SRC_ALPHA);
	...
	private void doTips() {
		...
		if (mDeadArrList.size() > 0) {
			String key = mDeadArrList.get(MathUtils.random(0,
					mDeadArrList.size() - 1));
			mSpark.setPosition(Integer.parseInt(key.substring(0, 1))
					* CELL_WIDTH + 8, Integer.parseInt(key.substring(1, 2))
					* CELL_HEIGHT + 8);
			mSpark2.setPosition(Integer.parseInt(key.substring(0, 1))
					* CELL_WIDTH + 4, Integer.parseInt(key.substring(1, 2))
					* CELL_HEIGHT + 4);
			...
			mSpark.addShapeModifier(new SequenceShapeModifier(
					new ScaleModifier(1.5f, 0.4f, 0.6f), new ScaleModifier(
							0.1f, 0.6f, 0f)));
			mSpark2.addShapeModifier(new SequenceShapeModifier(
					new ScaleModifier(1.5f, 0.5f, 1.1f), new ScaleModifier(
							0.1f, 1.1f, 0f)));
		} else {
			checkMapDead();
		}
				...
				if (mHashMap.get(getKey(j, i)).getState() == STATE_DEAD) {
					...
					for (v = 1; j + v < CELLS_VERTICAL
							&& mHashMap.get(getKey(j, i)).getStyle() == mHashMap
									.get(getKey(j, i + v)).getStyle()
							&& mHashMap.get(getKey(j, i)).getState() == mHashMap
									.get(getKey(j, i + v)).getState(); v++)
						;
					for (int z = v; z > 0; z--) {
	...
	private boolean checkDead(final JewelSprite sprite) {
		...
		if ((col - 1) >= 0) {
			...
			int v = 0;
			for (v = 1; col - 1 - v >= 0
					&& mHashMap.get(getKey(row, col - 1)).getStyle() == mHashMap
							.get(getKey(row, col - 1 - v)).getStyle()
					&& mHashMap.get(getKey(row, col - 1)).getState() == mHashMap
							.get(getKey(row, col - 1 - v)).getState(); v++)
				;
			if (v >= 3) {
				flag = true;
			}
		...
		if ((col + 1) < CELLS_HORIZONTAL) {
			...
			for (v1 = 1; col + 1 + v1 < CELLS_HORIZONTAL
					&& mHashMap.get(getKey(row, col + 1)).getStyle() == mHashMap
							.get(getKey(row, col + 1 + v1)).getStyle()
					&& mHashMap.get(getKey(row, col + 1)).getState() == mHashMap
							.get(getKey(row, col + 1 + v1)).getState(); v1++)
				;
			if (v1 >= 3) {
				flag = true;
			}
		...
		if ((row - 1) >= 0) {
			...
			int v2 = 0;
			for (v2 = 1; row - 1 - v2 >= 0
					&& mHashMap.get(getKey(row - 1, col)).getStyle() == mHashMap
							.get(getKey(row - 1 - v2, col)).getStyle()
					&& mHashMap.get(getKey(row - 1, col)).getState() == mHashMap
							.get(getKey(row - 1 - v2, col)).getState(); v2++)
				;
			if (v2 >= 3) {
				flag = true;
			}
		...
		if ((row + 1) < CELLS_VERTICAL) {
			...
			for (v3 = 1; row + 1 + v3 < CELLS_VERTICAL
					&& mHashMap.get(getKey(row + 1, col)).getStyle() == mHashMap
							.get(getKey(row + 1 + v3, col)).getStyle()
					&& mHashMap.get(getKey(row + 1, col)).getState() == mHashMap
							.get(getKey(row + 1 + v3, col)).getState(); v3++)
				;
			if (v3 >= 3) {
				flag = true;
			}
			...
			for (int j = 0; j < CELLS_HORIZONTAL - 2; j++) {
				if (mHashMap.get(getKey(j, i)).getState() == STATE_NORMAL) {
					for (k = 1; j + k < CELLS_HORIZONTAL
							&& mHashMap.get(getKey(j, i)).getStyle() == mHashMap
									.get(getKey(j + k, i)).getStyle()
							&& mHashMap.get(getKey(j, i)).getState() == mHashMap
									.get(getKey(j + k, i)).getState(); k++)
						;
					if (k >= 3) {
						this.addBonus();
						this.addScore(k);
				...
				if (mHashMap.get(getKey(i, j)).getState() == STATE_NORMAL) {
					for (k = 1; j + k < CELLS_VERTICAL
							&& mHashMap.get(getKey(i, j)).getStyle() == mHashMap
									.get(getKey(i, j + k)).getStyle()
							&& mHashMap.get(getKey(i, j)).getState() == mHashMap
									.get(getKey(i, j + k)).getState(); k++)
						;
					if (k >= 3) {
						this.addBonus();
						this.addScore(k);
	...
	private void changeChapter() {
		...
		TextureRegionFactory.createFromAsset(this.mBackgroundTexture, this,
				getRandomBG(), 0, 0);
	}
	...
	private void addScore(int fallCount) {
		...
		if (this.mScore < 9999999) {
			...
			Toast.makeText(getApplicationContext(), "恭喜你，你通关了！",
	...
	private void onStopGame() {
		Dialog dialog = new AlertDialog.Builder(this).setTitle("游戏提示")
	...
	private void submitScore() {
		...
		final View submitTextView = layoutInflater.inflate(R.layout.submit,
				null);
		final EditText mNameEditText = (EditText) submitTextView
				.findViewById(R.id.edittext_score);
							...
							public void onClick(DialogInterface dialog,
								...
								if (name.equals("") || name.trim().equals("")) {
									Toast.makeText(getApplicationContext(),
											"名字不能为空！", Toast.LENGTH_LONG)
											.show();
									submitScore();
								} else if (helper.isNameExist(name)) {
									Toast.makeText(getApplicationContext(),
											"该名已经存在！", Toast.LENGTH_LONG)
											.show();
									submitScore();
								} else {
									final int model;
									...
									final String rank = helper.queryrank(
											String.valueOf(mScore), model);
									helper.insertData(name, mScore, model,
											Integer.parseInt(rank) + 1);// 插入排行榜数据库
									dialog.cancel();
									toMenuView();
								}
							}
						})
				.setNeutralButton("取  消",
						new DialogInterface.OnClickListener() {
							public void onClick(DialogInterface dialog,
									int whichButton) {
								toMenuView();
